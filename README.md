...
Run the following command after activating the virtual environment:
		...
		python3 -m pip install langgraph langchain langchain-openai langchain-groq langchain-community langchain-tavily psycopg psycopg_pool python-dotenv tavily-python requests streamlit

		python3 -m pip install -U "psycopg[binary,pool]" langgraph-checkpoint-postgres
